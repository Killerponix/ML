...
def divValTrainSet(X,Y):
    ValSet = np.random.choice(X.shape[0],int(X.shape[0]*0.20),replace=False)
...
myANN = Sequential()
myANN.add(Dense(32, input_dim=12, activation='sigmoid'))
# myANN.add(Dropout(0.001))  # 20% der Neuronen werden deaktiviert
myANN.add(Dense(16, activation='sigmoid', use_bias=False))
# myANN.add(Dropout(0.01))  # 20% Dropout
myANN.add(Dense(8, activation='sigmoid', use_bias=False))
myANN.add(Dense(4, activation='sigmoid', use_bias=False))
myANN.add(Dense(2, activation='sigmoid', use_bias=False))
# myANN.add(Dropout(0.001))
m yANN.add(Dense(1, activation='linear', use_bias=False))
...
filename = 'yPredict4_Garb_Simon.csv'
